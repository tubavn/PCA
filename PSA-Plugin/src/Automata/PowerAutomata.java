...
//        PrintTransition();
    ...
    private  void GenerateAutomataFromFile(String filePath) throws IOException {
        ...
        while ((line = br.readLine()) != null) {
            ...
            States[count].name=part[0];
            for (int i = 1; i < part.length; i++) {
                Transition[count][i-1] = part[i];
            }
            //setup values and corefficient later
            count++;
        }
    }
    ...
    public Graph ToGrahp() {
        ...
        for (int i = 0; i < States.length; i++) {
            g.addVertex( States[i].name );
        }
        if(Transition!=null) {
            for (int i = 0; i < Transition.length; i++) {
                for (int j = 0; j < Transition[i].length; j++)
                    if (!Transition[i][j].equals( "null" )) {
                        g.addEdge( new Edge( Alpha[j] ), States[i].name, Transition[i][j] );
                        //  System.out.println(  States[i].name +"--------"+  Alpha[j]+ "-------" +Transition[i][j]  );
                    }
            }
        }
        return g;
